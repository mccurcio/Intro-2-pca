...
| :--------------------- | :-------------------------------------------------------------------------: |
| Centering | $\large f(x) ~=~ x - \bar x$ |
| Scaling between [0, 1] | $\large f(x) ~=~ \frac {x - min(x)} {max(x) - min(x)}$ |
| Scaling between [a, b] | $\large f(x) ~=~ (b - a)* \Large \frac {x - min(x)} {max(x) - min(x)} + a$ |
| Normalizing | $\large f(x) ~=~ \frac {x - mean(x)} {\sigma_x}$ |
...
xy_pca <- prcomp(cbind(x, y), scale = T)
...
knitr::include_graphics("Biplot1.annotated.png")
```
